...
export const Appointments = ({ setSelectedAppointment, appointments, setApptOpen, setReasignOpen, setCancelOpen }: AppointmentsProps) => {
  return (
    <ItemComponent title="Gestión de Citas" subtitle="Agenda, modifica o cancela tus citas">
      ...
      {appointments.filter(a => (a.appointmentState != 'completed' && a.appointmentState != 'paid')).map((c) => (
        <Box key={c.id} sx={{ borderLeft: `4px solid ${PALETA.azul}`, bgcolor: PALETA.welcomeBg, p: 2, borderRadius: "0 8px 8px 0", mb: 2 }}>
          ...
          {c.appointmentState == 'pending' &&

            <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
              <Button onClick={() => {
                setSelectedAppointment(c);
                setReasignOpen(true)
              }
              } sx={{ bgcolor: "#eee7e1", color: "#2c3e50", "&:hover": { bgcolor: "#dad6d3" } }}>Reasignar</Button>
              <Button onClick={() => {
                setSelectedAppointment(c);
                setCancelOpen(true)
              }
              } sx={{ bgcolor: "#ca370b", color: "#fff", "&:hover": { bgcolor: "#ac3315" } }}>Cancelar</Button>
            </Stack>
          }
        </Box>
      ))}
